...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Student*> zaneVect(auxStudents.size());
		for (size_t j = 0, i = 0; j <auxStudents.size(); j++, i++)
		{
			zaneVect[i] = &auxStudents[j];
		}

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareStudentPtr as the ordering relationship.

		sort(zaneVect.begin(), zaneVect.end(), compareStudentPtr);
		...
		for (size_t i = 0; i < students.size() && i < zaneVect.size(); i++)
		{
			students[i] = *zaneVect[i];
		}
