...
{
	numChanges = 0;
	seqArr = new ItemType[DEFAULT_MAX_ITEMS];
	sizeOfSeqArr = DEFAULT_MAX_ITEMS;
}
// Create an empty sequence (i.e., one with no items)

Sequence::Sequence(int initVal)
{
	numChanges = 0;
	seqArr = new ItemType[initVal];
	sizeOfSeqArr = initVal;
}
...
Sequence::Sequence(const Sequence& old)
{
	if (this != &old)
	{
		seqArr = new ItemType[old.sizeOfSeqArr];
...
Sequence& Sequence::operator=(Sequence& other)
{
	if (this != &other)
	{
		Sequence temp(other); //call of the copy constructor
		this->swap(temp); //swaps the member variables, pointers included
	}
	return *this;
}
//new default Destructor
Sequence::~Sequence()
{
	delete [] seqArr;
}
