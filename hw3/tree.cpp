...
{
	//need a way to when you find a match both go down the 'path' of that match and down other paths
	//each time you come across two that match go down a 'path' to see if match all three
		//if yes do that again with index of first shifted over
	if (n1 < 0 || n2 < 0)
		return 1;
	if (n2 == 0)
		return 1;
...
void separate(string a[], int n, string separator, int& firstNotLess, int& firstGreater)
{
	...
	//  Every element from position firstNotLess to firstUnknown-1 is == separator
...
void order(string a[], int n)
{
	...
	separate(a, n, a[0], firstNotLess, firstGreater);//split pile into two unsorted piles of about n/2 items
...
int main()
{
	...
	string arr2[3] = { "kyle", "cartman", "butters" };
